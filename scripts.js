...
function validateInteger(num, min = 0, max = Infinity) {
	return typeof num === "number" ? (min <= num) && (num <= max) : false;
}
...
function formatProduct(product, quantity = undefined) {
	return `${product.title} — ${quantity ? `${quantity}x` : ''}${formatPrice(product.price)} ${quantity ? `samtals ${formatPrice(product.price * quantity)}` : ''}`;
}
...
function cartInfo(cart) {
	...
	returnString += `Samtals: ${formatPrice(total)}`;
...
function showProducts() {
	for (let i = 0; i < products.length; i++) {
		...
		console.info(`#${i+1} ${product.title} — ${product.description} — ${formatPrice(product.price)}`);
	}
}
...
function addProductToCart() {
	...
	if (!idAsString) return console.error('Auðkenni má ekki vera tómt.');

	const id = Number(idAsString);
	if (!validateInteger(id, 0)) return console.error('Auðkenni vöru er ekki löglegt, verður að vera heiltala stærri en 0.');

	const product = products.find((e) => e.id === id);
	if (!product) return console.error('Vara fannst ekki.');

	const quantityAsString = prompt('Fjöldi:');
	if (!quantityAsString) return console.error('Fjöldi má ekki vera tómur.');

	const quantity = Number(quantityAsString);
	if (!validateInteger(quantity, 1, 99)) return console.error('Fjöldi er ekki löglegur, lágmark 1 og hámark 99.');

	const existingLine = cart.lines.find((e) => e.product === product.id);
	if (existingLine) {
		existingLine.quantity += quantity;
		console.info(`Vörufjöldi uppfærður:\n${formatProduct(product, existingLine.quantity)}`);
	} else {
		cart.lines.push({product: product.id, quantity});
 ...
 */
function checkout() {
	if (cart.lines.length < 1) return console.error('Karfan er tóm.');

	const name = prompt('Nafn:');
	if (!name) return console.error('Nafn má ekki vera tómt.');

	const address = prompt('Heimilisfang:');
	if (!address) return console.error('Heimilisfang má ekki vera tómt');
